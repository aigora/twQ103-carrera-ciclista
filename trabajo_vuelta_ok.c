...
void menu(Corredores *corredores)
{
	...
	do
	{
		...
		switch(opcion)
		{
			...
			sleep(1);     //permite volver al menú esperando un breve espacio de tiempo
		}
	} while(opcion!=7);
}
...
//contador auxiliar para poder hacer la funcion ordenar
...
void ordenar (Corredores *corredores, int last_dorsal)
{
  ...
  for (i = last_dorsal-2 ; flag==1 && i>= 0+1 ; i--)
  {
  	flag= 0;
    for (j = 0; j<= i-1 ; j++)
    {
...
void imprimir_tiempos(Corredores *corredores, int last_dorsal)
{
	...
	else
	{
		for(i=0; i<last_dorsal -1; i++)
		{
		    ...
		    fprintf(f,"%d h  ", corredores[i].tiempo.horas);
